...
QBertConditions = {
	...
	Win = function ()
		...
		for i = 1, #Board, 1 do
			row = Board[i]
			for j = 1, #row, 1 do
				if memory.readbyte(0x700+Board[i][j])%0x10 > 0 then 
		--			emu.print("Win not met (Missing touches @ " .. i .. "," .. j .. ")")
					return false 
				end
			end
		end
	...
	Update = function ()
		...
		for i = 1, #Board, 1 do
			...
			for j = 1, #row, 1 do
				...
				if (Board[i][j]>0) then Islands[i][j] = 1
				else Islands[i][j] = 0
				end
			end
		end
		...
		CurTurn.NumIslands = numIslands-1
...
QBertActions = {
	Down = {
		Priority = function()
			...
			if (CurTurn.Backtracks+CurTurn.NumIslands >= 4 or CurTurn.justBacktracked) then return 0 end
