...
function breakTest1()
	...
	if (frame < 73202 or frame > 73412) then return end
	if (RAMwrite[frame]==nil) then
		breakFlag = true
		return
	end
	
	value = memory.readbyte(0x64)
	if value ~= RAMwrite[frame] then 
		emu.print("Wrong position for enemy spawn #1: " .. value .. " instead of " .. RAMwrite[frame])
		breakFlag = true 
	else
		emu.print("Correct position for enemy spawn #1: " .. value)
	end
end

function breakTest2()
	frame = emu.framecount()
	if (frame < 73724 or frame > 73930) then return end
		if (RAMwrite[frame]==nil) then
		breakFlag = true
		return
	end
	
	value = memory.readbyte(0x67)
	if value ~= RAMwrite[frame] then
		emu.print("Wrong position for enemy spawn #2: " .. value .. " instead of " .. RAMwrite[frame])
		breakFlag = true 
	end	
end
...
function breakTest3()
	frame = emu.framecount()
	if (frame < 73939 or frame > 74149) then return end
	if (RAMwrite[frame]==nil) then
		breakFlag = true
		return
	end
	
	value = memory.readbyte(0x68)
	if value ~= RAMwrite[frame] then 
		emu.print("Wrong position for enemy spawn #3: " .. value .. " instead of " .. RAMwrite[frame])
		breakFlag = true 
	end
end

memory.registerwrite(0x64, 0x1, breakTest1)
memory.registerwrite(0x67, 0x1, breakTest2)
memory.registerwrite(0x68, 0x1, breakTest3)
