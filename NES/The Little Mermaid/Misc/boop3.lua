  ...
  for i = 3,15,1 do
   if bit.band(memory.readbyte(0x300+i),0x80)~=0 then
	...
	if memory.readbyte(0x310+i)==0x30 and heldobject ~= i then
	 gui.box(objectxpos-screenxpos-4, objectypos-roomscrolly-4, objectxpos-screenxpos+4, objectypos-roomscrolly+4)
	end
    if heldobject == i or memory.readbyte(0x310+i)==1 then
     if memory.readbyte(0x310+i)==1 then
      gui.box(objectxpos-screenxpos-4, objectypos-roomscrolly-4, objectxpos-screenxpos+4, objectypos-roomscrolly+4)
   	 end
	 gui.box(objectxpos-screenxpos-1, objectypos-roomscrolly-1, objectxpos-screenxpos+1, objectypos-roomscrolly+1,"black")
     gui.pixel(objectxpos-screenxpos, objectypos-roomscrolly,"green")
	 else
     boxtype = bit.band(memory.readbyte(0x03C0+i),0x1F)
	 height   = rom.readbyte(0x1FB70+boxtype)
	 width    = rom.readbyte(0x1FB90+boxtype)
     gui.box(objectxpos-screenxpos-width, objectypos-roomscrolly-height, objectxpos-screenxpos+width, objectypos-roomscrolly+height)
     end 
   end 
  end
  
 ariel   = {memory.readbyte(0x0330)+256*memory.readbyte(0x0340)-screenxpos,memory.readbyte(0x0360)-240*yposadjust*memory.readbyte(0x0370)-roomscrolly,"red"}
 bubble1 = {memory.readbyte(0x0331)+256*memory.readbyte(0x0341)-screenxpos,memory.readbyte(0x0361)-240*yposadjust*memory.readbyte(0x0371)-roomscrolly,"red"}
 bubble2 = {memory.readbyte(0x0332)+256*memory.readbyte(0x0342)-screenxpos,memory.readbyte(0x0362)-240*yposadjust*memory.readbyte(0x0372)-roomscrolly,"red"}
